...
export class FileHandle implements fs.promises.FileHandle {
  ...
  public static async create(filename: string): Promise<FileHandle> {
    const filehandle = await fs.promises.open(filename, 'rw+');
...
/**
 * This is a rewrite of the GoLang os.TempFile method (https://golang.org/pkg/io/ioutil/#TempFile), with additional
 * functionality.
 *
 ...
 * @returns {Promise<TempFileInterface|void>} Returns Promise if callback is not defined, void if callback is defined.
 */
export const tempFileOfSize = async (
  options: TempFileOfSizeOptions,
  callback?: TempFileValidationCallback,
): Promise<FileHandle | void> => {
  const fhos = (await tempFile(options)) as FileHandle;
  let size = bytes(options.size);
  const chunkSize = bytes('1MB');

  while (size > chunkSize) {
    // const written =
    await fhos.write(crs({length: chunkSize /*, type: 'base64'*/}), 'utf-8');
    // TODO: should test if written.bytedWritten === chunkSize
    size -= chunkSize;
  }

  if (size > 0) {
    // const written =
    await fhos.write(crs({length: chunkSize}), 'utf-8');
    // TODO: should test if written.bytedWritten === size
  }

  fhos.close();

  const promise = FileHandle.create(fhos.name());
  if (!callback) {
    return promise;
  }
...
/**
 * This is a rewrite of the GoLang os.TempDir method (https://golang.org/pkg/io/ioutil/#TempDir), with additional
 * functionality.
 *
  ...
  if (!localOptions.default) {
    const name =
      localOptions.pattern.indexOf('*') >= 0
        ? localOptions.pattern.replace('*', shortid.generate())
        : localOptions.pattern + shortid.generate();
    promise = fs.promises.mkdir(path.join(localOptions.dir, name), {
      recursive: true,
    });
  } else {
...
export const tempDirWithFiles = async (
